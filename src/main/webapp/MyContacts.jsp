<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page import="com.pkg.Dao.ContactsDao, java.sql.ResultSet,com.pkg.POJO.ContactDetails,com.pkg.POJO.DbPojo"%>
...
<%
 ...
 List<ContactDetails> contactList = (List<ContactDetails>)request.getAttribute("contactsList");
 %>
...
<body>	
		...
		<tr>
			...
			<td>
				<button type="button" class="action-btn"
					onclick="openModal('<%= contact.getContactId() %>', '<%= contact.getNickName() %>', 
                        '<%= contact.getContactDob() %>', '<%= contact.getGender() %>', 
                        '<%= contact.getPlace() %>', '<%= contact.getContactEmail()%>', 
                        '<%= contact.getContactPhone() %>')">
					Edit
				</button>
				<form action="DeleteContactServlet" method="post" style="display:inline;">
	...
	<% } else { %>
        <h2 style="background: linear-gradient(120deg, #c471f5, #fcfcfc);">No contacts added</h2>
    <% } %>
	...
	<div id="updateModal" class="modal">
		<div class="modal-content">
			...
			<form action="UpdateContactServlet" method="post">
				...
				<input type="text" name="nickName" id="modalNickName" required>
				<br>
				<br>
				
				<label for="contactDob">Date of Birth:</label>
				<input type="date" name="contactDob" id="modalContactDob" >
				<br>
				<br>
	...
	function openModal(contactId, nickName, dob, gender, place, email, phone) {
