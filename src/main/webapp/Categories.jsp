...
<%@page import="java.sql.ResultSet,com.pkg.POJO.Categories,com.pkg.POJO.CategoryMap"%>
...
<div class="category-container">
    ...
    <div class="category-actions">
        ...
        <form action="DeleteCategoryServlet" method="post">
            <input type="hidden" name="categoryId" value="<%=category.getCategoryID()%>">
            <input type="submit" value="Delete" class="delete-button" onclick="return confirm('Are you sure you want to delete this category?');">
        </form>
    </div>
</div>
<%
...
<div class="form-section">
    <form action="AddCategoryServlet" method="post">
        ...
        <h4>Add Members:</h4>
        <%
        List<ContactDetails> contacts = (List<ContactDetails>)request.getAttribute("contactList");
                    for(ContactDetails contact: contacts){
        %>
        <div class="contact-checkbox">
            <input type="checkbox" name="selectedContacts" value="<%= contact.getContactId() %>"> <%= contact.getNickName() %><br>
        </div>
        <%
            }
        %>
